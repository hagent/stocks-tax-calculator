...
async function parseTransactions(rates) {
  const lines = (await fs.readFile('stocks_transactions.txt', 'utf8'))
    .split('\n');
  // .slice(0, 10);
  let year;
  let date;
...
async function main() {
  ...
  const transactions = await parseTransactions(rates);
  ...
  console.log(stocks.filter(x => x.type === 'sell' && x.date.getFullYear() === 2021).reduce((acc, x) => acc + x.income, 0));
